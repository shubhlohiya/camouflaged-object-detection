...
class CamObjDataset(data.Dataset):
    ...
    #obtain the image from the index
    ...
    #convert loaded image to three channel (rgb)
    def rgb_loader(self, path):
        with open(path, 'rb') as f:
            img = Image.open(f)
            return img.convert('RGB')

    #convert loaded image to single channel
...
#for more robust loading of test data
class test_loader_faster(data.Dataset):
    ...
    #obtain the image from the specified index
    def __getitem__(self, index):
    ...
    #convert the image to three channel (rgb)
    def rgb_loader(self, path):
        with open(path, 'rb') as f:
            img = Image.open(f)
            return img.convert('RGB')
...
def get_loader(image_root, gt_root, batchsize, trainsize, shuffle=True, num_workers=0, pin_memory=True):
    #choose num_workers = 0 for more stable training
